...
```{r}
proporcion_aeropuerto <- vuelos %>%
  mutate(cancelados = (is.na(tailnum))) %>%
  group_by(origin) %>%
  summarise(proporcion_cancelados = mean(cancelados),media_dep_delay = mean(dep_delay, na.rm = TRUE),media_arr_delay = mean(arr_delay, na.rm = TRUE)) %>% ungroup()
ggplot(proporcion_aeropuerto) +
  geom_line(aes(x = media_dep_delay, y = proporcion_cancelados, col= proporcion_cancelados))
```
